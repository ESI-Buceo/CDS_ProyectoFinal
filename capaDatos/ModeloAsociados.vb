...
Public Class ModeloAsociados
    ...
    Public Function FiltrarPatologiasPorSintomas(ByVal sqlSintomas As String, cantidadSintomas As Integer)
        ...
        Comando.CommandText = "SELECT a.idPatologia, p2.nombre , p2.descripcion , p2.ponderacion 
                            FROM asociados a 
                            JOIN patologia p2 on p2.id = a.idPatologia and p2.activo = 1 
                            JOIN sintoma s ON s.id = a.IdSintoma  AND s.activo 
                            WHERE a.IdSintoma IN (" & sqlSintomas & ") 
                            GROUP BY a.idPatologia HAVING count(a.idPatologia ) >='" & cantidadSintomas & "'"
